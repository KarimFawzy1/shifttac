import 'package:flutter/material.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:shifttac/features/tiki_taka/domain/models/tiki_game_status.dart';
import 'package:shifttac/features/tiki_taka/presentation/state/tiki_taka_cubit.dart';
import 'package:shifttac/features/tiki_taka/presentation/widgets/tiki_taka_completion_dialog.dart';

import 'tiki_taka_dialog_test_support.dart';

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  late TikiTakaTestDatabaseHandle databaseHandle;
  late TikiTakaCubit cubit;
  var restartTapped = false;
  var goHomeTapped = false;

  setUpAll(ensureTikiTakaDaoTestInit);

  setUp(() async {
    resetTikiDialogVisibilityForTest();
    restartTapped = false;
    goHomeTapped = false;
    databaseHandle = await openTikiTakaTestDatabase();
    cubit = seedTikiCubit(
      handle: databaseHandle,
      status: TikiGameStatus.completed,
      elapsed: const Duration(minutes: 4, seconds: 30),
      hearts: 2,
    );
    cubit.pauseTimer();
  });

  tearDown(() async {
    await cubit.close();
    await databaseHandle.close();
    resetTikiDialogVisibilityForTest();
  });

  Future<void> pumpDialog(WidgetTester tester) async {
    await tester.pumpWidget(
      wrapTikiDialogTest(
        cubit: cubit,
        child: TikiTakaCompletionDialog.forTest(
          routeAnimation: const AlwaysStoppedAnimation<double>(1),
          elapsed: const Duration(minutes: 4, seconds: 30),
          hearts: 2,
          onRestart: () => restartTapped = true,
          onGoHome: () => goHomeTapped = true,
        ),
      ),
    );
    await tester.pump();
  }

  testWidgets('shows completion copy and stats without continue', (tester) async {
    await pumpDialog(tester);

    expect(find.text('Board complete!'), findsOneWidget);
    expect(find.text('4:30'), findsOneWidget);
    expect(find.text('2'), findsOneWidget);
    expect(find.text('Restart'), findsOneWidget);
    expect(find.text('Go Home'), findsOneWidget);
    expect(find.text('Continue Playing'), findsNothing);
  });

  testWidgets('restart action fires callback', (tester) async {
    await pumpDialog(tester);

    await tester.tap(find.byKey(TikiTakaCompletionDialog.restartButtonKey));
    await tester.pump();

    expect(restartTapped, isTrue);
    expect(goHomeTapped, isFalse);
  });
}
